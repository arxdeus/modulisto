...
/// Signature of callbacks that holds `T` data.
typedef PayloadCallback<T> = void Function(T);
...
abstract class ValueListenable<T> {
  ...
  void addListener(PayloadCallback<T> listener);

  /// Remove a previously registered closure from the list of closures that the
  /// object notifies.
  void removeListener(PayloadCallback<T> listener);
}

/// A class that can be extended or mixed in that provides a change notification
/// API using [PayloadCallback] for notifications.
mixin class ValueNotifier<T> implements ValueListenable<T> {
  int _count = 0;
  // The _listeners is intentionally set to a fixed-length _GrowableList instead
  // of const [].
  static final List<PayloadCallback<Object?>?> _emptyListeners = List<PayloadCallback<Object?>?>.filled(0, null);
  List<PayloadCallback<T>?> _listeners = _emptyListeners;
  int _notificationCallStackDepth = 0;
  int _reentrantlyRemovedListeners = 0;
  bool _debugDisposed = false;

  /// Used by subclasses to assert that the [ValueNotifier] has not yet been
  /// disposed.
  static bool debugAssertNotDisposed(ValueNotifier<Object?> notifier) {
    assert(
      () {
  ...
  @mustCallSuper
  void addListener(PayloadCallback<T> listener) {
    assert(ValueNotifier.debugAssertNotDisposed(this), '');

    if (_count == _listeners.length) {
      if (_count == 0) {
        _listeners = List<PayloadCallback<T>?>.filled(1, null);
      } else {
        final newListeners = List<PayloadCallback<T>?>.filled(_listeners.length * 2, null);
  ...
  void _removeAt(int index) {
    ...
    if (_count * 2 <= _listeners.length) {
      final newListeners = List<PayloadCallback<T>?>.filled(_count, null);
  ...
  void notifyListeners(T value) {
    ...
    if (_notificationCallStackDepth == 0 && _reentrantlyRemovedListeners > 0) {
      ...
      if (newLength * 2 <= _listeners.length) {
        ...
        final newListeners = List<PayloadCallback<T>?>.filled(newLength, null);
