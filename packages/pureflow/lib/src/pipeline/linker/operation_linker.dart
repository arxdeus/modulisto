...
import 'package:pureflow/src/operation.dart';
...
extension OperationLinkerPipelineExt on PipelineRef {
  OperationPipelineLinker<T> operationOnType<T>(Function operationId) =>
      OperationPipelineLinker._(
        operationId,
        this,
      );
}
...
class OperationPipelineLinker<T>
    implements PipelineLinker<Symbol, T>, PipelineRefHost {
  ...
  void bind(FutureOr<void> Function(MutatorContext context, T value) handler) {
    ...
    trigger.listen((value) {
      // if (pureflowSettings.debugReportOperationTypeMismatch) {
      //   assert(
      //     value.runtimeType == T,
